    ...
    <>
      <div className="summary-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: 24 }}>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ color: 'var(--income)' }}>${fmt(totalDeposited)}</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Total Withdrawn</div>
          <div className="stat-value" style={{ color: 'var(--expense)' }}>${fmt(totalWithdrawn)}</div>
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div className="card-body">
            <form onSubmit={handleSubmit}>
              ...
              <div className="input-group">
                <label>Note <span style={{ color: 'var(--muted-foreground)', fontWeight: 400 }}>(optional)</span></label>
                <input
                  type="text"
              ...
              <button type="submit" className={`btn btn-primary hysa-submit-btn${type === 'withdrawal' ? ' withdrawal' : ''}`} style={{ width: '100%' }}>
            ...
            {transactions.length === 0 ? (
              ...
              <table className="stock-table">
                ...
                <tbody>
                  {transactions.map(tx => (
                    <tr key={tx.id}>
                      <td style={{ color: 'var(--muted-foreground)', fontSize: '0.85rem' }}>{tx.date}</td>
                      <td>
                      ...
                      <td style={{ color: 'var(--muted-foreground)', fontSize: '0.85rem' }}>{tx.note || '—'}</td>
                      <td>
